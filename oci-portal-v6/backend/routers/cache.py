...
async def get_cache_config_endpoint(
    ...
    """
    Get cache configuration for the frontend.
    
    Returns:
        Dictionary with cache names and their configuration
    """
    try:
        config = {
            "DV920": get_cache_config("DV920"),
            "PY920": get_cache_config("PY920"),
            "DM920": get_cache_config("DM920")
        }
        return {"ok": True, "config": config}
    except Exception as e:
        app_logger.error(f"Failed to load cache config: {str(e)}")
