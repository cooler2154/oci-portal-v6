...
async function loadCacheConfig() {
  try {
    _cacheConfig = await api('GET', '/clearCaches/config') || {};
    renderCacheCards();
  } catch(err) {
...
// function renderCacheCards() {
  ...
  // // Don't regenerate the grid — it already has styled cards from tab_cache.html
  // // Just show/hide based on permissions
// }
function renderCacheCards() {
  const canConfig = _currentUser?.role === 'admin';
  const btnCfg = document.getElementById('cache-config-btn');
...
async function execClearCache() {
  ...
  try {
    const result = await api('POST', `/clearCaches/${name}`);
